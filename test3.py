...
# Initialize lists to hold all detected circles and their radii
all_circles = []
all_radii_mm = []
...
for color_name, (lower, upper) in color_ranges.items():
    ...
    if circles is not None:
        ...
        for i in circles[0, :]:
            ...
            all_radii_mm.append(radius_mm)

            cv2.circle(image, center, radius, (0, 255, 0), 2)
            cv2.circle(image, center, 2, (0, 0, 255), 3)
            cv2.putText(image, f"R: {radius_mm:.2f}mm", (center[0] - 40, center[1] - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.5,
                        (255, 0, 0), 1)
...
# Save the radii to a CSV file
output_csv_path = 'radii_in_mm.csv'
with open(output_csv_path, mode='w', newline='') as file:
    writer = csv.writer(file)
    writer.writerow(["Circle Number", "Center X (pixels)", "Center Y (pixels)", "Radius (mm)"])
    for i, ((center, radius), radius_mm) in enumerate(zip(all_circles, all_radii_mm)):
        writer.writerow([i + 1, center[0], center[1], f"{radius_mm:.2f}"])

# Display the image with circles and radius annotations
